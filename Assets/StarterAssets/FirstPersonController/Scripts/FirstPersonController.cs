...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		private void Move()
		{
			...
			// if player is in dialogue, set target speed to 0
			if (DialogueManager.Instance.InDialogue()) targetSpeed = 0.0f;
		...
		{
			Debug.Log($"Teleporting player from {transform.position} to {newPosition}");
			// Disable CharacterController to avoid collision issues while teleporting
			_controller.enabled = false;

			// Move player transform to new position
			transform.position = newPosition;

			// Re-enable CharacterController
			_controller.enabled = true;

			// Optional: Reset vertical velocity to avoid fall glitches after teleport
